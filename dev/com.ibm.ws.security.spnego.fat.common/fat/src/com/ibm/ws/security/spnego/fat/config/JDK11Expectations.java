 ...
 *
 */
public class JDK11Expectations implements JDKExpectationTestClass {
...
///////////////////////////////////////////////////////
    ...
    //on JDK 11 the bad spn test case is expected to fail
    private static final String responseCheckForBadSpn1 = "Bad SPN Test #1 failed";
    private static final String responseCheckForBadSpn2 = "Bad SPN Test #2 failed";
...
////////////////////
    ...
    public void KDCErr_NoSupportForEncryptionType(LoginException e) {
        assertTrue("Expected exception was thrown but error message was unexpected.", e.getMessage().contains("KDC has no support for encryption type (14)"));
    }
...
//GSS credentials will reflect the original user, not the mapped user
    ...
    public void unsuccesfulSpnegoServletCall(String response, String ownerInformation) {
        assertFalse("Response should NOT contain GSS credentials but credentials were found.", response.contains(SPNEGOConstants.JDK11_GSS_CREDENTIAL_STRING));
...
//////////////////////////////////
    ...
    public void responseContainsInvalidCredentialError(String response) {
        assertTrue("The response should have had Invalid credentials but it did not.", response.contains("No valid credentials provided"));
    }
...
//        assertTrue("Response should contain GSS credentials but none were found.", response.contains(SPNEGOConstants.JDK11_GSS_CREDENTIAL_STRING));
    ...
    //to remove the middle portions of a line inside the response.
    @Override
    ...
    public void spnegoInvalidCredential(GSSException e) {
        assertTrue("Expected exception was thrown but error message was unexpected.", e.getMajorString().contains("No valid credentials"));
    }
...
///////////////////////////////////
    ...
    public void s4u2Proxy_NoClassDefFoundError(String response) {
        ...
        if (response.contains("java.lang.NoClassDefFoundError: com/ibm/websphere/security/s4u2proxy/SpnegoHelper")) {
            assertTrue("The response should have had a NoClassDefFoundError but it did not.",
                       true);
        } else if (response.contains("java.lang.NoClassDefFoundError: com.ibm.websphere.security.s4u2proxy.SpnegoHelper")) {
            assertTrue("The response should have had a NoClassDefFoundError but it did not.",
                       true);
        } else {
            assertTrue("The response should have had a NoClassDefFoundError but it did not.",
    ...
    public void s4u2_validateKerberosAndGSSCred(String response) {
        ...
        assertTrue("The response should have had Client SpNegoCredElement listed", response.contains("sun.security.jgss.spnego.SpNegoCredElement"));
    }
