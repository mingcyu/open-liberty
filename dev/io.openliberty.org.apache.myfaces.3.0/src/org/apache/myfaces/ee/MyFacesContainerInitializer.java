...
public class MyFacesContainerInitializer implements ServletContainerInitializer {
    ...
    /**
     * If the servlet mapping for the FacesServlet is added dynamically,
     * Boolean.TRUE
    ...
    /**
     * If the servlet mapping for the FacesServlet is found on the ServletContext,
     * Boolean.TRUE
     * is stored under this key in the ServletContext.
     */
    ...
    /**
     * If the flag is true, the algoritm skip jar scanning for faces-config files to
     * check if the current
     * application requires FacesServlet to be added dynamically (servlet spec 3).
     * This param can be set using
     * a system property with the same name too.
     */
    @JSFWebConfigParam(since = "2.2.10", expectedValues = "true, false", defaultValue = "false", tags = "performance")
    ...
    public void onStartup(Set<Class<?>> clazzes, ServletContext servletContext) throws ServletException {
        ...
        String supportManagedBeans = servletContext.getInitParameter(MyfacesConfig.INIT_PARAM_SUPPORT_MANAGED_BEANS);
        if (supportManagedBeans == null || Boolean.TRUE.toString().equals(supportManagedBeans)) {
            ...
            // in < 2.3 it the ExternalContext#applicationMap was used but we have no access
            // here
        ...
        // 3. A filename ending in .faces-config.xml is found in the META-INF directory
        // of a jar in the
        // application's classpath.
        // 4. The jakarta.faces.CONFIG_FILES context param is declared in web.xml or
        // web-fragment.xml.
        // 5. The Set of classes passed to the onStartup() method of the
        ...
        if ((clazzes != null && !clazzes.isEmpty()) || isFacesConfigPresent(servletContext)) {
            ...
            String[] mappings = isAutomaticXhtmlMappingDisabled(servletContext) ? FACES_SERVLET_MAPPINGS
                    : FACES_SERVLET_FULL_MAPPINGS;
    ...
    /**
     * Checks if the <code>INITIALIZE_ALWAYS_STANDALONE</code> flag is ture in
     * <code>web.xml</code>.
     * If the flag is true, this means we should not add the FacesServlet, instead
     * we want to
    ...
    /**
     * Checks if the <code>INITIALIZE_SCAN_JARS_FOR_FACES_CONFIG</code> flag is true
     * in <code>web.xml</code>.
     * If the flag is true, this means we should scan app jars for
     * *.faces-config.xml before adding
    ...
    private boolean isFacesConfigPresent(ServletContext servletContext) {
        try {
            ...
            // 4. The jakarta.faces.CONFIG_FILES context param is declared in web.xml or
            // web-fragment.xml.
            // check for alternate faces-config files specified by
            ...
            // Skip this scan - for performance - if INITIALIZE_SKIP_JAR_FACES_CONFIG_SCAN
            // is set to true
            // 2. A faces-config.xml file is found in the META-INF directory of a jar in the
            // application's classpath.
            // 3. A filename ending in .faces-config.xml is found in the META-INF directory
            ...
            // To do this properly it is necessary to use some SPI interfaces MyFaces
            // already has, to
    ...
    /**
     * Checks if the class represented by className implements
