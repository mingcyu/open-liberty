...
/**
 * Singleton Instance of the Utility class which handles Sip Naming Requests
 * from WAS components
 * 
 */
public class SipResolver implements SipResolverTransportListener {
    ...
    /**
     * Maximum size of request cache. If this is exceeded service flushes all
     * outstanding request and starts over.
     */
    ...
    /**
     * Standard MAX Ethernet MTU 1500 octets (does not include the ethernet header
     * 14 octets and 4 octets CRC)
     */
    ...
    /**
     * recommendation by rfc 2671 is for a lower value to prevent ICMP from
    ...
    /**
     * DS method for removing the ByteBufferConfiguration. This is a required
    ...
    /**
     * Returns reference to ByteBufferConfiguration
     * 
     * @return
     */
    protected ByteBufferConfiguration getByteBufferConfiguration() {
        if (c_logger.isEventEnabled()) {
            c_logger.event("wsbbmgr = " + wsbbmgr);
        }
        return wsbbmgr;
    }

    /**
     * Returns reference to NettyFramework
     * 
     * @return
     */
    protected NettyFramework getNettyBundle() {
        if (c_logger.isEventEnabled()) {
            c_logger.event("framework = " + framework);
        }
        return framework;
    }
     ...
     */
    public static synchronized SipResolver createResolver(Vector<InetSocketAddress> nameServers,
            Properties containerProps, NettyFramework f) {
        framework = f;
        ...
        if (_resolver == null) {
            ...
            if (val != null && val.length() > 0) {
                try {
                    ...
                    if (c_logger.isTraceDebugEnabled())
                        c_logger.traceDebug(
                                "SipResolverService: initialize: invalid SIP_RFC3263_DNS_FAILURE_DETECTION_SINGLE_QUERY_TIMEOUT_SEC "
                                        + val);
                }
                if (c_logger.isTraceDebugEnabled())
                    c_logger.traceDebug(
                            "SipResolverService: initialize: SIP_RFC3263_DNS_FAILURE_DETECTION_SINGLE_QUERY_TIMEOUT_SEC "
            ...
            if (val != null && val.length() > 0) {
                try {
                    ...
                    if (c_logger.isTraceDebugEnabled())
                        c_logger.traceDebug(
                                "SipResolverService: initialize: invalid SIP_RFC3263_DNS_FAILURE_DETECTION_ALLOWED_FAILURES "
                                        + val);
                }
                if (c_logger.isTraceDebugEnabled())
                    c_logger.traceDebug(
                            "SipResolverService: initialize: SIP_RFC3263_DNS_FAILURE_DETECTION_ALLOWED_FAILURES "
                                    + _allowed_failures);
            }
            ...
            if (val != null && val.length() > 0) {
                try {
                    ...
                    if (c_logger.isTraceDebugEnabled())
                        c_logger.traceDebug(
                                "SipResolverService: initialize: invalid SIP_RFC3263_DNS_FAILURE_DETECTION_WINDOW_SIZE_MIN "
                                        + val);
                }
                if (c_logger.isTraceDebugEnabled())
                    c_logger.traceDebug(
                            "SipResolverService: initialize: SIP_RFC3263_DNS_FAILURE_DETECTION_WINDOW_SIZE_MIN "
                                    + _window_interval_sec);
            }
            ...
            if (val != null && val.length() > 0) {
                try {
                    ...
                    if (c_logger.isTraceDebugEnabled())
                        c_logger.traceDebug(
                                "SipResolverService: initialize: invalid SIP_RFC3263_DNS_FAILURE_DETECTION_WINDOW_SIZE_INTERVAL_SEC "
                                        + val);
                }
                if (c_logger.isTraceDebugEnabled())
                    c_logger.traceDebug(
                            "SipResolverService: initialize: SIP_RFC3263_DNS_FAILURE_DETECTION_WINDOW_SIZE_INTERVAL_SEC "
                                    + _window_size_min);
            }
            ...
            if (tempObject != null) {
                ...
                if (c_logger.isTraceDebugEnabled()) {
                    if (tempObject instanceof Boolean)
                        c_logger.traceDebug(
                                "SipResolver: createResolver:  <" + SIP_USE_PRECISE_SYSTEM_TIMER + ">  is a Boolean");
                    else
                        c_logger.traceDebug("SipResolver: createResolver:  <" + SIP_USE_PRECISE_SYSTEM_TIMER
                                + ">  is NOT a Boolean");
                    c_logger.traceDebug("SipResolver: createResolver:  <" + SIP_USE_PRECISE_SYSTEM_TIMER
                            + ">  is set to " + tempObject);
                }
            } else {
                if (c_logger.isTraceDebugEnabled())
                    c_logger.traceDebug(
                            "SipResolver: createResolver:  <" + SIP_USE_PRECISE_SYSTEM_TIMER + ">  is not set");
            }
    ...
    public SipResolverEvent resolve(DnsMessage dnsRequest, SipResolverListener listener, boolean doTCP) {
        ...
        try {
            ...
            synchronized (_requestPending) {
                if (_requestPending.size() > MAX_REQUEST_CACHE_SIZE) {
                    IllegalStateException e = new IllegalStateException(
                            "Request Cache overflowed. Shutdown SIP Resolver service.");
                ...
                if (c_logger.isTraceDebugEnabled()) {
                    c_logger.traceDebug(
                            "SipResolver: resolve put in _requestPending: " + new Integer(dnsRequest.getId()));
                }
            ...
            synchronized (_requestPending) {
                if (c_logger.isTraceDebugEnabled()) {
                    c_logger.traceDebug(
                            "SipResolver: resolve remove from _requestPending: " + new Integer(dnsRequest.getId()));
                }
        ...
        try {
            ...
            if (request != null) {
                ...
                if (endTime - startTime > _dnsRequestTimeout) {
                    if (c_logger.isTraceDebugEnabled()) {
                        c_logger.traceDebug("SipResolver: responseReceived: DNS responded after : "
                ...
                case Dns.NAME_ERROR:
                    ...
                    if (c_logger.isTraceDebugEnabled())
                        c_logger.traceDebug(
                                "SipResolver: responseReceived: notify listener with NAMING_EXCEPTION event");
    ...
    public void transportError(Exception exception, SipResolverTransport transport) {
        ...
        try {
            ...
            // call back, using this thread, into this object's resolve(....) method with
            // the
            ...
            synchronized (_requestPending) {
                ...
                if (c_logger.isTraceDebugEnabled()) {
                    c_logger.traceDebug("SipResolver: transportError: clearing out pending events - # events: "
                            + _requestPending.size());
                }
    ...
    public void cancelRequest(DnsMessage msg, boolean isTimeout) {
        ...
        synchronized (_requestPending) {
            if (c_logger.isTraceDebugEnabled()) {
                c_logger.traceDebug(
                        "SipResolver: cancelRequest: remove from _requestPending: " + new Integer(msg.getId()));
            }
        ...
        if (isTimeout && request != null) {
            ...
            if ((endTime - startTime) > _dnsRequestTimeout) {
                if (c_logger.isTraceDebugEnabled()) {
                    c_logger.traceDebug("SipResolver: responseReceived: DNS responded after : " + (endTime - startTime)
                            + " millisec");
                }
                int total = _eventsCounter.reportEvent();
                if (total > _allowed_failures && _enableSecondaryDNS) {
    ...
    public void transportFailed(Exception exception, SipResolverTransport transport) {
        ...
        // once NAMING_TRANSPORT_FAILURE is called, the listener could, using this
        // thread
        ...
        synchronized (_requestPending) {
            ...
            if (c_logger.isTraceDebugEnabled()) {
                c_logger.traceDebug("SipResolver: transportFailed: clearing out pending events - # events: "
                        + _requestPending.size());
            }
